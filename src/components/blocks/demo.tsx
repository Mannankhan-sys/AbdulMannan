                            ...
                            <MarbleCard key={i} delay={i * 2.5} className="flex flex-col h-full text-black p-8 md:p-10">
                                <div className="flex-1">
                                    <div className="flex items-center gap-3 mb-6 text-[#c5a059]">
                                        {p.icon}
                                        <span className="text-xs font-mono tracking-widest font-bold uppercase border border-[#c5a059]/30 px-3 py-1 rounded-full">{p.category}</span>
                                    </div>
                                    <h4 className="text-2xl lg:text-3xl font-black mb-4 tracking-tight uppercase leading-snug">
                                        {p.title.split(' ')[0]} <GoldText>{p.title.split(' ').slice(1).join(' ')}</GoldText>
                                    </h4>
                                    <p className="text-neutral-600 text-sm lg:text-base leading-relaxed mb-8 font-medium">
                                        {p.desc}
                                    </p>
                                    <div className="flex flex-wrap gap-2 mb-8">
                                        {p.tech.map((t, idx) => (
                                            <span key={idx} className="text-[10px] font-mono px-3 py-1.5 rounded-md bg-neutral-100 text-neutral-600 font-bold uppercase border border-neutral-200">
                                                {t}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                <div className="flex flex-col sm:flex-row items-center justify-between pt-6 border-t border-neutral-100 gap-4">
                                    <a href={p.demo} className="w-full sm:flex-1 text-center py-3.5 bg-black text-white hover:bg-[#c5a059] transition-colors rounded-xl text-xs font-black tracking-widest uppercase flex items-center justify-center gap-2 shadow-sm">
                                        <ExternalLink className="w-4 h-4" /> Run Demo
                                    </a>
                                    <a href={p.github} className="w-full sm:flex-1 text-center py-3.5 bg-white border-2 border-neutral-100 text-black hover:border-[#c5a059] hover:bg-neutral-50 transition-colors rounded-xl text-xs font-black tracking-widest uppercase flex items-center justify-center gap-2">
...
const PortfolioDemo = () => {
    return (
        <div className="min-h-screen bg-black">
            <ScrollExpandMedia
                mediaType="image"
                mediaSrc="/galaxy-bg-hd.jpg"
                bgImageSrc="/galaxy-bg-hd.jpg"
                title="THE CREATION"
